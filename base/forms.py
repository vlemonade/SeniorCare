...
class register_form(ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'suffix': forms.TextInput(attrs={'placeholder': 'Suffix'}),
            'age': forms.NumberInput(attrs={'placeholder': 'Age', 'required': True}),
            #'sex': forms.TextInput(attrs={'placeholder': 'Sex', 'required': True, 'class': 'form-control form-select'}),
             'sex': forms.Select(choices=senior_list.SEX_CHOICES, attrs={'class': 'form-select'}),
            ...
            'OSCA_ID': forms.NumberInput(attrs={'placeholder': 'OSCA_ID', 'required': True, 'id': 'id_osca_id'}),
            'senior_image': forms.FileInput(attrs={'required': False, 'id': 'image'}), 
            'phone_number': forms.TextInput(attrs={'placeholder': 'Contact No.', 'required': True}),
            'allowance_type': forms.Select(attrs={'class': 'form-control'}),  
            'allowance_amount': forms.NumberInput(attrs={'placeholder': 'Allowance Amount', 'required': True})
        }
